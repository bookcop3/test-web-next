  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/user/products_H/history_H`);
      ...
      <div className="grid  md:grid-cols-4 md:gap-5">
        <div>
          <ul>
            ...
            <li>
              <Link href="/user/product-history">
        ...
        <div className="overflow-x-auto md:col-span-3">
          <div className="flex justify-between">
            <h1 className="mb-4 text-xl">Product History</h1>
          </div>
          {loading ? (
            <div>Loading...</div>
          ) : error ? (
...
ProductHistoryScreen.auth = true;
